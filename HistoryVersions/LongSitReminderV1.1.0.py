"""
Author: Big Panda
Created Time: 19.03.2025 11:08
Modified Time: 19.03.2025 11:08
Description:
    bug 修复：
        目前，我们鼠标悬停在 pet 上时， 动画移动过程会被暂停，但是，由于 QTimer 还在计时，如果我们保持悬停超过三秒，则 pet 会继续自动移动，从而导致逻辑错乱，我们应该在鼠标悬停在 pet 上时直接停止 QTimer，等待移开后再打开 QTimer
    新增：
        拖动时 pet 的表情切换，由于拖动前必须点击，因此，我们通过点击对这个表情进行替换，点击结束后，即鼠标释放后再替换回来
"""
import os
import sys
import random
...
class DesktopPet(QWidget):
    ...
    def enterEvent(self, event):
        ...
        # 新增： 当鼠标悬停在 pet 上时，第一时间停止 QTimer
        self.stop_move()
    ...
    def leaveEvent(self, event):
        ...
        # 新增： 当鼠标离开 pet 时，第一时间判断之前是否已经存在 animation 状态，如果存在，就继续之前的移动状态，如果不存在，不做任何操作
        # 并且第二时间重新打开 QTimer 计时，进入下一次随机移动计时
        self.start_move()
