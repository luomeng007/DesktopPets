"""
Author: Big Panda
Created Time: 19.03.2025 9:35
Modified Time: 19.03.2025 9:35
Description:
    实现 pet 的自动移动和停止移动，
        1. 加载后自动开始随机移动
        2. 可以手动关闭随机移动
...
class DesktopPet(QWidget):
    ...
    def show_context_menu(self, pos):
        ...
        # 当前 QMenu 和 QAction 的生命周期我们都选择由当前的 First_level_menu 决定，因为当前这些功能都在这个 First_level_menu 下
        # 我们此时没有必要让这些组件的生命周期由主窗口管理
        # 无论我们选择这些组件的生命周期由谁来管理，必要的添加组件过程不可少，比如 First_level_menu.addMenu(second_level_menu_music)
        # 和 second_level_menu_music.addAction(action_play_music)
