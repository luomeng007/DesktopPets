...
Description:
    新增：
        实现首次运行程序时宠物位于屏幕中心
        打开应用程序后的 tooltip 提示语句
    bug 修复：
        之前，当鼠标在 pet 上悬停后，关闭了 Timer，导致随机移动计时消失了。离开后，pet 仍然处于静止状态，如果悬停之前，pet 处于随机移动状态，那么鼠标离开后，应该继续进行随机移动计时

    仍旧存在一些打包上面的 bug 需要修复。比如不使用 --onefile 模式进行打包时
"""
import os
import sys
import random
...
class DesktopPet(QWidget):
    def __init__(self):
        ...
        self.path_pet_model_default = resource_path(r"../Main/Pets/MengJi_White.png")
        self.path_pet_model_Drag = resource_path(r"../Main/Pets/MengJi_White_Drag.png")
        ...
        self.color_yellow.triggered.connect(partial(self.change_pet_color, resource_path(r'../Main/Pets/MengJi_Orange.png')))
        self.color_white.triggered.connect(partial(self.change_pet_color, resource_path(r'../Main/Pets/MengJi_White.png')))
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() == Qt.MouseButton.LeftButton:
            ...
            self.path_pet_model_Drag = resource_path(''.join([self.path_pet_model_default.split('.')[0].split('/')[-1], '_Drag.', self.path_pet_model_default.split('.')[1]]))
